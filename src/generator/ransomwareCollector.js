...
const COUNTRY = {
  ...
  MC: 'Monaco', US: 'États-Unis', GB: 'Royaume-Uni', DE: 'Allemagne', ES: 'Espagne',
...
export async function collectRansomware(hours = 24) {
  ...
  const recent = data
    ...
    .sort((a, b) => new Date(b.attackdate || b.discovered) - new Date(a.attackdate || a.discovered));
  ...
  return [...fr, ...intl].map((v) => {
    ...
    const desc = (v.description || '').replace(/^\[AI generated\]\s*/i, '').trim();
    const lines = [
    ...
    return {
      ...
      severity: 'news',
      section: 'attacks',
      vendor: null, cvss: null, exploited: false, ransomware: true,
      body: lines.join(' '),
      detail: lines.join('\n\n'),
      action: null,
      sources: [{ url: v.url || 'https://www.ransomware.live', label: 'ransomware.live' }],
