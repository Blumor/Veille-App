...
export async function collectANSSI(hours = 720) {
  ...
  const entries = [...xml.matchAll(/<url>([\s\S]*?)<\/url>/g)]
    ...
    .filter(
      (e) =>
        /\/actualites\/[^?#]+\/?$/.test(e.loc) &&
        e.lm &&
        new Date(e.lm) >= cutoff,
    )
  ...
  const results = await Promise.allSettled(
    entries.map(async (e) => {
      ...
      const section =
        /menace|alerte|attaque|vuln[ée]rabilit|campagne|incident/.test(t)
          ? "attacks"
          : "culture";
