...
const URL =
  "https://api.github.com/advisories?per_page=100&sort=published&type=reviewed";
const UA = {
  "User-Agent": "VeilleApp/1.0",
...
export async function collectGitHubAdvisories(hours = 168) {
  ...
  return data
    .filter(
      (a) =>
        SEV[a.severity] && a.published_at && new Date(a.published_at) >= cutoff,
    )
    .map((a) => {
      ...
      return {
        ...
        exploited: false,
        body: summary,
        detail: [summary, vendor ? `Paquet affecté : ${vendor}.` : ""]
          .filter(Boolean)
          .join("\n\n"),
        action: null,
