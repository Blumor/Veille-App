...
const PERIOD = {
  daily: {
    window: "les dernières 24 heures",
    noun: "journée",
    of: "de la journée",
  },
  weekly: {
    window: "les 7 derniers jours",
    noun: "semaine",
    of: "de la semaine",
  },
...
const FRANCE_RE =
  /\bfrance\b|fran[çc]ais|française|\banssi\b|\bcnil\b|cert-fr|gendarmerie|\bparis\b|hexagone|\boiv\b|secnumcloud/i;
...
function regionOf(item) {
  ...
  if (FRANCE_RE.test(`${item.title} ${item.body || ""} ${item.detail || ""}`))
    return "fr";
  return "intl";
}
...
const EXPLOIT_RE =
  /actively exploit|exploited in (the wild|attacks)|under (active )?exploitation|being exploited|exploitation active|activement exploit|zero.?day|0.?day/i;
...
export function itemAuthority(item) {
  const weights = (item.sources || []).map(
    (s) => AUTHORITY[s.label] ?? DEFAULT_AUTH,
  );
...
function scoreVuln(item) {
  ...
  if (item.exploited)
    s += 25; // KEV : exploitation avérée
  else if (item.epss != null)
    s += Math.round(item.epss * 20); // EPSS : probabilité, ≤ +20
  else if (EXPLOIT_RE.test(`${item.title} ${item.detail || item.body || ""}`))
    s += 12;
...
const IMPACT_LEXICON = [
  ...
  {
    re: /actively exploit|exploited in the wild|activement exploit|exploitation active|in[- ]the[- ]wild/,
    w: 10,
  },
  { re: /\brce\b|remote code execution|ex[ée]cution de code/, w: 9 },
  { re: /supply.?chain|cha[îi]ne d'approvisionnement|supply chain/, w: 9 },
  {
    re: /nation.?state|state.?sponsored|\bapt\d*\b|[ée]tatique|espionnage|spyware/,
    w: 8,
  },
  {
    re: /critical infrastructure|infrastructure critique|\boiv\b|\bscada\b|\bics\b|h[ôo]pital|hospital|sant[ée]|[ée]nergie|energy|nucl[ée]aire/,
    w: 8,
  },
  { re: /ransomware|ran[çc]ongiciel/, w: 6 },
  {
    re: /data breach|fuite de donn[ée]es|\bleak\b|vol de donn[ée]es|exfiltrat|piratage/,
    w: 5,
  },
  { re: /backdoor|porte d[ée]rob[ée]e|\bwiper\b|rootkit|botnet/, w: 5 },
  {
    re: /\bgouvernement|government|d[ée]fense|defense|minist[èe]re|military|arm[ée]e/,
    w: 5,
  },
  {
    re: /urgent|emergency|alerte|patch now|correctif urgent|sans d[ée]lai/,
    w: 4,
  },
];
const SUPERLATIVE =
  /largest|biggest|record|massive|unprecedented|first.?ever|jamais vu|sans pr[ée]c[ée]dent|historique|g[ée]ant/;
...
function magnitudeBonus(t) {
  ...
  const m = t.match(
    /(\d[\d.,\s]{3,})\s*(records|comptes|accounts|users|utilisateurs|victim|victimes|personnes|patients|clients)/,
  );
  if (m) {
...
function contentImpact(item) {
  const t =
    `${item.title} ${item.body || ""} ${item.detail || ""}`.toLowerCase();
  let c = 0;
...
function scoreNews(item) {
  ...
  return Math.max(
    0,
    Math.min(100, reliability + impact + recencyBonus(item, 5, 2)),
  );
}
...
export function composeItem(item) {
  ...
  const detail =
    fullSrc && !fullSrc.startsWith(lead)
      ? `${lead}\n\n${fullSrc}`
      : fullSrc || lead;
...
export function composeSummary(all, type) {
  ...
  if (attacks.length)
    parts.push(
      `${attacks.length} ${plural(attacks.length, "incident")} ou ${plural(attacks.length, "compromission")}`,
    );
  if (culture.length)
    parts.push(
      `${culture.length} ${plural(culture.length, "signal", "signaux")} de fond`,
    );

  // Produits / éditeurs les plus cités
  const vendorCount = {};
  for (const i of all)
    if (i.vendor) vendorCount[i.vendor] = (vendorCount[i.vendor] || 0) + 1;
  ...
  if (topVendors.length)
    summary += ` Acteurs et produits les plus exposés : ${listFr(topVendors)}.`;
  return summary;
}
