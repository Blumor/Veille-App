...
const ATTACK_RE =
  /breach|leak|ransomware|attack|malware|phishing|incident|intrusion|botnet|ddos|stealer|infostealer|data theft|exfiltrat|extortion|espionage|spyware|hacked|data sale|attaque|attaqu|fuite|piratage|piraté|rançongiciel|ran[çc]on|compromis|vol de donn|donn[ée]es vol|exfiltrat|extorsion|espionnage|cyberattaque|hame[çc]onnage/;

function guessSeverity(title, body, cve) {
  const t = (title + " " + body).toLowerCase();
  if (
    /critical|critique|actively exploit|exploitation active|zero.?day|0.?day|cvss[:\s]*(9|10)/.test(
      t,
    )
  )
    return "critical";
  if (
    cve ||
    /\bhigh\b|élevé|exploit|rce|remote code|injection|bypass|privilege esc/.test(
      t,
    )
  )
    return "high";
...
function guessSection(title, body, cve) {
  ...
  if (
    cve ||
    /cve-|vuln[ée]rabilit|vulnerabilit|faille|patch|correctif|exploit|overflow|injection|\brce\b|bypass|zero.?day|0.?day/.test(
      t,
    )
  )
    return "vulns";
