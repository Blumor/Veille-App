...
export function todayISO() {
  const d = new Date();
  return new Date(d - d.getTimezoneOffset() * 60_000)
    .toISOString()
    .slice(0, 10);
}
...
const frDay = (iso) =>
  D(iso).toLocaleDateString("fr-FR", { day: "numeric", month: "long" });
const frDayY = (iso) =>
  D(iso).toLocaleDateString("fr-FR", {
    day: "numeric",
    month: "long",
...
const TITLES = {
  daily: (d) => `Rapport quotidien - ${frFull(d)}`,
  weekly: (d) => `Rapport hebdomadaire - ${weekRange(d)}`,
  monthly: (d) => `Rapport mensuel - ${monthCovered(d)}`,
};
...
export async function generateReport(type, dateISO = todayISO()) {
  if (!HOURS[type]) {
    throw new Error(
      `Type inconnu : ${type} (attendu daily | weekly | monthly)`,
    );
  }
  ...
  const pick = (arr) => {
    ...
    const frExtra = arr
      .filter((it) => it.region === "fr" && !inTop.has(it))
      .slice(0, FR_EXTRA);
  ...
  if (!sections.length) {
    throw new Error(
      "Aucune section non vide — pas assez d'articles collectés.",
    );
  }
