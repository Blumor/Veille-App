...
const safeUrl = (u) => {
  try {
    return /^https?:$/.test(new URL(u).protocol) ? u : "#";
  } catch {
    return "#";
  }
};
...
function reportLabel(type, dateISO) {
  ...
  if (type === "monthly") {
    ...
    prev.setDate(1);
    prev.setDate(0);
    const m = prev.toLocaleDateString("fr-FR", {
      month: "long",
      year: "numeric",
    });
...
const scoreClass = (n) =>
  n >= 80 ? "sc-crit" : n >= 60 ? "sc-high" : n >= 40 ? "sc-mid" : "sc-low";

const sectionMeta = (type, id) =>
  (STATE.sections[type] || []).find((s) => s.id === id) ?? {
    label: id,
    icon: "•",
  };
...
function bindSearch(rep) {
  ...
  const apply = () => {
    ...
    const c = $("#searchCount");
    if (c)
      c.textContent = STATE.query.trim()
        ? `${getFilteredItems(rep).length} résultat(s)`
        : "";
  };
  input.oninput = apply;
  apply();
...
const inRegion = (it) =>
  STATE.region === "all" || (it.region || "intl") === STATE.region;
...
function bylineHTML(item) {
  ...
  const chips = labels
    .slice(0, 3)
    .map((l) => `<span class="fi-src">${esc(l)}</span>`)
    .join("");
  const more =
    labels.length > 3
      ? `<span class="fi-src-more">+${labels.length - 3}</span>`
      : "";
  return chips + more;
}
  ...
  const facts = [
    {
      label: "Importance",
      value: `${score}/100`,
      cls: `fact-score ${scoreClass(score)}`,
    },
    {
      label: "Gravité",
      value: SEV_LABELS[(item.severity || "news").toLowerCase()] || "Info",
    },
  ];
  if (item.cve) facts.push({ label: "CVE", value: item.cve });
  const corro = item.corroboration || 1;
  if (corro >= 2)
    facts.push({ label: "Recoupement", value: `${corro} médias` });
  const d = frDateFull(item.pubDate);
  if (d) facts.push({ label: "Date", value: d });
  return `<div class="detail-facts">
    ${facts
      .map(
        (f) => `<div class="fact ${f.cls || ""}">
...
function renderDetail(item) {
  ...
  const sources = item.sources?.length
    ? item.sources
    : item.url
      ? [{ url: item.url, label: shortHost(item.url) }]
      : [];
  const linksHTML = sources
    .map(
      (
        s,
      ) => `<a class="src-link" href="${esc(safeUrl(s.url))}" target="_blank" rel="noopener noreferrer">
  ...
  return `<div class="detail-view fade-in">
    ...
    ${
      item.action
        ? `<div class="detail-section detail-action-block">
...
async function init() {
  renderEmpty();
  try {
    STATE.sections = (await api("/api/config")).sections;
  } catch {
